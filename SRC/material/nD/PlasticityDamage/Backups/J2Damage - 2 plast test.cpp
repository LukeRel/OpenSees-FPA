...
static int df = 1;	// Turn this off for debug in plastic state only
...
// Plasticity integration routine 1 [by Quan Gu and Zhijian Qiu]
int J2Damage::plasticity1() {
...
// Plasticity integration routine 2 [by Edward Love]
void J2Damage::plasticity2()
{
	const double tolerance = (1.0e-8) * sig_y;
	...
	dev_strain = Eps;
	for (i = 0; i < 3; i++)	dev_strain(i, i) -= (1.0/3.0 * trace);

	//compute the trial deviatoric stresses
	Matrix Eps_p_k(3,3);
	for (int i = 0; i < 3;i++) Eps_p_k(i, i) = strain_p_k(i);
	Eps_p_k(0, 1) = strain_p_k(3); Eps_p_k(1, 2) = strain_p_k(4); Eps_p_k(2, 0) = strain_p_k(5);
	Eps_p_k(1, 0) = strain_p_k(3); Eps_p_k(2, 1) = strain_p_k(4); Eps_p_k(0, 2) = strain_p_k(5);
	...
	phi = norm_tau - sqrt(2.0/3.0) * Hk*alpha_k;
	...
	if (phi > 0.0) { //plastic
	   ...
	   //solve for gamma 
		...
		while (fabs(resid) > tolerance) {
			...
			resid = norm_tau - (2.0 * G) * gamma - G * Hk*(alpha_k + sqrt(2.0/3.0) * gamma);
			tang = -(2.0 * G) - 2.0/3.0 * Hk;
			gamma -= (resid / tang);
			iteration_counter++;
		...
		Matrix Eps_p = Eps_p_k + gamma * normal;
		alpha = alpha_k + sqrt(2.0/3.0) * gamma;

		//recompute deviatoric stresses 
		dev_stress = (2.0 * G) * (dev_strain - Eps_p_k);

		//compute the terms for plastic part of tangent
		theta = (2.0 * G) + 2.0/3.0 * Hk;
		theta_inv = 1.0 / theta;
	...
	Matrix Sig = dev_stress;
	for (i = 0; i < 3; i++)	Sig(i, i) += K * trace;

	for (int i = 0; i < 3;i++) stress(i) = Sig(i,i);
	stress(3) = Sig(0,1); stress(4) = Sig(1,2); stress(5) = Sig(2,0);
	...
	for (ii = 0; ii < 6; ii++) {
		for (jj = 0; jj < 6; jj++) {
			...
			//elastic terms
			tens_tang[i][j][k][l] = G * IbunI[i][j][k][l];
			tens_tang[i][j][k][l] += (2.0 * K) * IIdev[i][j][k][l];

			//plastic terms 
			tens_tang[i][j][k][l] += c2 * NbunN;
			tens_tang[i][j][k][l] += c3 * (IIdev[i][j][k][l] - NbunN);

			//minor symmetries 
			tens_tang[j][i][k][l] = tens_tang[i][j][k][l];
			tens_tang[i][j][l][k] = tens_tang[i][j][k][l];
			tens_tang[j][i][l][k] = tens_tang[i][j][k][l];

			tangent(ii, jj) = tens_tang[i][j][k][l];
...
int J2Damage::setTrialStrain(const Vector& pStrain)
{
	...
	double trace = strain(0) + strain(1) + strain(2);
	Vector I2(6);                // unity vector order 2
	I2.Zero();
	for (int i = 0; i < 3; i++)
		I2(i) = 1.0;
	...
	*/
	...
	dstrain = strain - strain_k;
	//Vector dstrain1 = dstrain - dstrain_p;
	double dD = D - D_k;
	...
	Vector dD_dstrain(6);
	for (int i = 0;i < 6;i++) {
		if (dD == 0) dD_dstrain(i) = 0;
		else dD_dstrain(i) = dD/dstrain(i);
	}
	Matrix Ct(6, 6);
	for (int i = 0;i < 6;i++)	for (int j = 0;j < 6;j++) {
		Ct(i,j) = Dm1sq* tangent(i,j) - 2 * (1 - D) * tangent_e(i,j) * strain_e(j) * dD_dstrain(j);
	}
	stress = stress_k + Ct * dstrain;
	tangent = Ct;
	*/
	/*
