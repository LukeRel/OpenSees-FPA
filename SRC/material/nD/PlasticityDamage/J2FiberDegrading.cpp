...
// $Source: /usr/local/cvs/OpenSees/SRC/material/nD/J2FiberDegrading.cpp,v $
...
// components 22, 33, and 23 are condensed out.
// Based on J2 plasticity with L. Parente simple degradation.
// Requires:
//  - t0 initiation time
//  - t  analysis time
//  - v  corrosion speed = dx(t)/dt

#include <J2FiberDegrading.h>           
#include <Channel.h>
#include <string.h>
#include <math.h>
#include <float.h>
...
Vector J2FiberDegrading::sigma(3);
Matrix J2FiberDegrading::D(3,3);

void *
OPS_J2FiberDegrading(void)
{
  ...
  if (numArgs < 6) {
    opserr << "Want: nDMaterial J2FiberDegrading $tag $E $v $sigmaY $Hiso $Hkin <De>" << endln;
    return 0;	
  }
  ...
  if (OPS_GetInt(&numData, iData) != 0) {
    opserr << "WARNING invalid integer tag: nDMaterial J2FiberDegrading \n";
    return 0;
  }
  ...
  if (OPS_GetDouble(&numData, dData) != 0) {
    opserr << "WARNING invalid data: nDMaterial J2FiberDegrading : " << iData[0] <<"\n";
    return 0;
  }  
  
  theMaterial = new J2FiberDegrading(iData[0], dData[0], dData[1], dData[2], dData[3], dData[4], dData[5]);

  if (dData[5] > 0) opserr << "Degradation parameter for J2FiberDegrading material received. De = " << dData[5] << endln;
  ...
  alphan(0.0), alphan1(0.0), dg_n1(0.0)
{
  ...
  // Degrading mechanical parameters
  double alpha = 0.2;
...
int
J2FiberDegrading::setTrialStrain (const Vector &strain)
{
  Tepsilon = strain;
...
int
J2FiberDegrading::setTrialStrain (const Vector &strain, const Vector &rate)
{
  Tepsilon = strain;
...
int
J2FiberDegrading::setTrialStrainIncr (const Vector &strain)
{
  return 0;
}

int
J2FiberDegrading::setTrialStrainIncr (const Vector &strain, const Vector &rate)
{
  return 0;
}

const Matrix&
J2FiberDegrading::getTangent (void)
{
  ...
  if (F < -100*DBL_EPSILON) {
    ...
    if (iter == maxIter) {
      //opserr << "J2FiberDegrading::getTangent -- maxIter reached " << R.Norm() << endln;
    }
...
const Matrix&
J2FiberDegrading::getInitialTangent (void)
{
...
const Vector&
J2FiberDegrading::getStress (void)
{
  double G = 0.5*E/(1.0+nu);
  ...
  if (F < -100*DBL_EPSILON) {
    ...
    if (iter == maxIter) {
      //opserr << "J2FiberDegrading::getStress -- maxIter reached " << R.Norm() << endln;
    }
...
const Vector&
J2FiberDegrading::getStrain (void)
{
  return Tepsilon;
}
...
int
J2FiberDegrading::revertToLastCommit (void)
{
  epsPn1[0] = epsPn[0];
  epsPn1[1] = epsPn[1];
  epsPn1[2] = epsPn[2];
...
int
J2FiberDegrading::revertToStart (void)
{
  Tepsilon.Zero();
...
NDMaterial*
J2FiberDegrading::getCopy (void)
{
  J2FiberDegrading *theCopy =
    new J2FiberDegrading (this->getTag(), E, nu, sigmaY, Hiso, Hkin, De);

  return theCopy;
}

NDMaterial*
J2FiberDegrading::getCopy (const char *type)
{
...
const char*
J2FiberDegrading::getType (void) const
{
  return "BeamFiber";
}

int
J2FiberDegrading::getOrder (void) const
{
  return 3;
}

const Vector&
J2FiberDegrading::getStressSensitivity(int gradIndex, bool conditional)
{
...
int
J2FiberDegrading::commitSensitivity(const Vector &depsdh, int gradIndex, int numGrads)
{
  if (SHVs == 0) {
...
int
J2FiberDegrading::sendSelf (int commitTag, Channel &theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "J2FiberDegrading::sendSelf -- could not send Vector\n";
    return res;
  }
...
int
J2FiberDegrading::recvSelf (int commitTag, Channel &theChannel, 
				    FEM_ObjectBroker &theBroker)
{
  ...
  if (res < 0) {
   opserr << "J2FiberDegrading::recvSelf -- could not recv Vector\n";
   return res;
  }
...
void
J2FiberDegrading::Print (OPS_Stream &s, int flag)
{
...
int
J2FiberDegrading::setParameter(const char **argv, int argc,
			    Parameter &param)
{
...
int 
J2FiberDegrading::updateParameter(int parameterID, Information &info)
{ 
  switch(parameterID) {
  case 1:
    E = info.theDouble;
...
int
J2FiberDegrading::activateParameter(int paramID)
{
  parameterID = paramID;
