...
class DPDamageFiber : public NDMaterial
{
public:

	DPDamageFiber(int tag, double _E, double _nu, // Parameters
	...
	DPDamageFiber(const DPDamageFiber&);
	virtual ~DPDamageFiber();

	const char* getClassType(void) const { return "DPDamageFiber"; };
...
private:
	...
	double sig_t;   // Yield strength
	double sig_c;   // Yield strength
