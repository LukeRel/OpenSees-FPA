...
// NDFiberSectionIS3d modified to include initial strains.
// Initial fiber strains are defined in the Tcl input script and
...
// long term effects.
// Deformations are set within the Tcl script.
...
ID NDFiberSectionIS3d::code(6);

void* OPS_NDFiberSectionIS3d()
{
    int numData = OPS_GetNumRemainingInputArgs();
    if(numData < 1) {
	opserr<<"insufficient arguments for NDFiberSectionIS3d\n";
	return 0;
    }
    ...
    int num = 30;
    return new NDFiberSectionIS3d(tag, num, computeCentroid);
}

// constructors:
NDFiberSectionIS3d::NDFiberSectionIS3d(int tag, int num, Fiber** fibers, double a, bool compCentroid) :
    SectionForceDeformation(tag, SEC_TAG_NDFiberSectionIS3d),
    ...
    parameterID(0), dedh(6)
{
    if (numFibers != 0) {
        ...
        if (theMaterials == 0) {
            opserr << "NDFiberSectionIS3d::NDFiberSectionIS3d -- failed to allocate Material pointers";
            exit(-1);
        }
        ...
        if (matData == 0) {
            opserr << "NDFiberSectionIS3d::NDFiberSectionIS3d -- failed to allocate double array for material data\n";
            exit(-1);
        }
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theMaterials[i] == 0) {
                opserr << "NDFiberSectionIS3d::NDFiberSectionIS3d -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
...
NDFiberSectionIS3d::NDFiberSectionIS3d(int tag, int num, double a, bool compCentroid): 
    SectionForceDeformation(tag, SEC_TAG_NDFiberSectionIS3d),
    ...
    parameterID(0), dedh(6)
{
	...
	for (int i = 0; i < sizeFibers; i++) {
	    ...
	    matData[i*4+1] = 0.0;
	    matData[i*4+2] = 0.0;
        matData[i*4+3] = 0.0; // initial strains
	    theMaterials[i] = 0;
	}    
    }
...
NDFiberSectionIS3d::NDFiberSectionIS3d(int tag, int num, NDMaterial **mats, SectionIntegration &si, double a, bool compCentroid):
  SectionForceDeformation(tag, SEC_TAG_NDFiberSectionIS3d),
  numFibers(num), sizeFibers(num), theMaterials(0), matData(0),
  Abar(0.0), QyBar(0.0), QzBar(0.0), yBar(0.0), zBar(0.0), computeCentroid(compCentroid),
  alpha(a), sectionIntegr(0), e(6), s(0), ks(0), 
  parameterID(0), dedh(6)
{
  ...
  if (sectionIntegr == 0) {
    opserr << "Error: NDFiberSectionIS3d::NDFiberSectionIS3d: could not create copy of section integration object" << endln;
    exit(-1);
  }
...
NDFiberSectionIS3d::NDFiberSectionIS3d():
  SectionForceDeformation(0, SEC_TAG_NDFiberSectionIS3d),
...
int
NDFiberSectionIS3d::addFiber(Fiber &newFiber)
{
  ...
  if(numFibers == sizeFibers) {
	  ...
	  opserr <<"NDFiberSectionIS3d::addFiber -- failed to allocate Fiber pointers\n";
	  return -1;
      }
      ...
      newMatData[4*i+3] = matData[4*i+3]; // initial strains
      }
	  ...
	  newArray[i] = 0;
	  newMatData[4*i] = 0.0;
	  newMatData[4*i+1] = 0.0;
	  newMatData[4*i+2] = 0.0;
      newMatData[4*i+3] = 0.0;
      }
  ...
  if (theMaterials[numFibers] == 0) {
    opserr <<"NDFiberSectionIS3d::addFiber -- failed to get copy of a Material\n";
    return -1;
  }
...
//      0  0 0       0 sqrt(a)  y]
int
NDFiberSectionIS3d::setTrialSectionDeformation (const Vector &deforms)
{
  int res = 0;
  ...
  if (sectionIntegr != 0) {
      sectionIntegr->getFiberLocations(numFibers, yLocs, zLocs);
      sectionIntegr->getFiberWeights(numFibers, fiberArea);
      // sectionIntegr->getFiberInitialStrains(numFibers, iStrains);
  }
  else {
      for (int i = 0; i < numFibers; i++) {
          // Positions and area:
  ...
  for (int i = 0; i < numFibers; i++) {
      ...
      // Determine fiber strain and set it
...
const Vector&
NDFiberSectionIS3d::getSectionDeformation(void)
{
  return e;
}

const Matrix&
NDFiberSectionIS3d::getInitialTangent(void)
{
...
const Matrix&
NDFiberSectionIS3d::getSectionTangent(void)
{
  return *ks;
}

const Vector&
NDFiberSectionIS3d::getStressResultant(void)
{
  return *s;
}

SectionForceDeformation*
NDFiberSectionIS3d::getCopy(void)
{
  NDFiberSectionIS3d *theCopy = new NDFiberSectionIS3d ();
  ...
  if (numFibers != 0) {
    ...
    if (theCopy->theMaterials == 0) {
      opserr <<"NDFiberSectionIS3d::getCopy -- failed to allocate Material pointers\n";
      exit(-1);
    }
    ...
    if (theCopy->matData == 0) {
      opserr << "NDFiberSectionIS3d::getCopy -- failed to allocate double array for material data\n";
      exit(-1);
    }
	...
	opserr <<"NDFiberSectionIS3d::getCopy -- failed to get copy of a Material";
	exit(-1);
      }
    }  
  }
...
const ID&
NDFiberSectionIS3d::getType ()
{
  return code;
}

int
NDFiberSectionIS3d::getOrder () const
{
  return 6;
}

int
NDFiberSectionIS3d::commitState(void)
{
  int err = 0;
...
int
NDFiberSectionIS3d::revertToLastCommit(void)
{
  int err = 0;
...
int
NDFiberSectionIS3d::revertToStart(void)
{
...
int
NDFiberSectionIS3d::sendSelf(int commitTag, Channel &theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr <<  "NDFiberSectionIS3d::sendSelf - failed to send ID data\n";
    return res;
  }    
...
int
NDFiberSectionIS3d::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr <<  "NDFiberSectionIS3d::recvSelf - failed to recv ID data\n";
    return res;
  }    
  this->setTag(data(0));
  ...
  if (data(1) != 0) {
	...
	if (theMaterials == 0) {
	  opserr <<"NDFiberSectionIS3d::recvSelf -- failed to allocate Material pointers\n";
	  exit(-1);
	}
	...
	if (matData == 0) {
	  opserr <<"NDFiberSectionIS3d::recvSelf  -- failed to allocate double array for material data\n";
	  exit(-1);
	}
      }
    }
...
void
NDFiberSectionIS3d::Print(OPS_Stream &s, int flag)
{
  s << "\nNDFiberSectionIS3d, tag: " << this->getTag() << endln;
  s << "\tSection code: " << code;
...
Response*
NDFiberSectionIS3d::setResponse(const char **argv, int argc, OPS_Stream &output)
{
...
int 
NDFiberSectionIS3d::getResponse(int responseID, Information &sectInfo)
{
...
int
NDFiberSectionIS3d::setParameter(const char **argv, int argc, Parameter &param)
{
  if (argc < 1)
    return -1;
...
int
NDFiberSectionIS3d::updateParameter(int paramID, Information &info)
{
  switch(paramID) {
  case 1:
...
int
NDFiberSectionIS3d::activateParameter(int paramID)
{
  parameterID = paramID;
...
const Vector &
NDFiberSectionIS3d::getSectionDeformationSensitivity(int gradIndex)
{
  return dedh;
}

const Vector &
NDFiberSectionIS3d::getStressResultantSensitivity(int gradIndex, bool conditional)
{
  static Vector ds(6);
...
const Matrix &
NDFiberSectionIS3d::getInitialTangentSensitivity(int gradIndex)
{
...
int
NDFiberSectionIS3d::commitSensitivity(const Vector& defSens,
