...
void* OPS_UniaxialFiber2d()
{
    ...
    return new UniaxialFiber2d(numUniaxialFiber2d++,*theMat,data[2],data[0], data[3], data[4], data[5]);
}
...
UniaxialFiber2d::UniaxialFiber2d(int tag, 
                                 ...
                                 double Area, double position, double Eps0, double Beta, double Shear_y):
                                 Fiber(tag, FIBER_TAG_Uniaxial2d),
                                 theMaterial(0), area(Area), y(-position), eps0(Eps0), beta(Beta), shear_y(Shear_y)
{
...
UniaxialFiber2d::UniaxialFiber2d(): Fiber(0, FIBER_TAG_Uniaxial2d),
                                    theMaterial(0), area(0), y(0.0), eps0(0.0), beta(0.0), shear_y(0.0)
{
  if (code(0) != SECTION_RESPONSE_P) {
    code(0) = SECTION_RESPONSE_P;
    code(1) = SECTION_RESPONSE_MZ;
...
UniaxialFiber2d::getCopy (void)
{
   ...
   UniaxialFiber2d *theCopy = new UniaxialFiber2d (this->getTag(), 
                                                   *theMaterial, area, -y, eps0, beta, shear_y);
   return theCopy;
}  
