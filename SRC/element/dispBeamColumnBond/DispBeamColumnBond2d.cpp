...
// Created: May 2024
//
// Description: This file contains the class definition for DispBeamColumnBond2d.
// This is a displacement-based beam-column element with bond-slip. It requires
// an additional degree of freedom per node per slipping element.

#include <DispBeamColumnBond2d.h>
#include <Node.h>
...
Matrix DispBeamColumnBond2d::K(6,6);
Vector DispBeamColumnBond2d::P(6);
double DispBeamColumnBond2d::workArea[100];

void* OPS_DispBeamColumnBond2d()
{
    ...
    Element *theEle =  new DispBeamColumnBond2d(iData[0],iData[1],iData[2],secTags.Size(),sections,
					    *bi,*theTransf,mass,cmass);
    delete [] sections;
    return theEle;
}

void* OPS_DispBeamColumnBond2d(const ID &info)
{
    // data
    int iData[5];
    int numData;
...
int OPS_DispBeamColumnBond2d(Domain& theDomain, const ID& elenodes, ID& eletags)
{
    if(OPS_GetNumRemainingInputArgs() < 2) {
	...
	theEle = new DispBeamColumnBond2d(--currTag,elenodes(2*i),elenodes(2*i+1),secTags.Size(),
...
DispBeamColumnBond2d::DispBeamColumnBond2d(int tag, int nd1, int nd2,
				   int numSec, SectionForceDeformation **s,
				   BeamIntegration& bi,
				   CrdTransf &coordTransf, double r, int cm)
:Element (tag, ELE_TAG_DispBeamColumnBond2d), 
  ...
  Q(6), q(3), rho(r), cMass(cm), parameterID(0)
{
  ...
  if (theSections == 0) {
    opserr << "DispBeamColumnBond2d::DispBeamColumnBond2d - failed to allocate section model pointer\n";
    exit(-1);
  }
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    // Check allocation
    if (theSections[i] == 0) {
      opserr << "DispBeamColumnBond2d::DispBeamColumnBond2d -- failed to get a copy of section model\n";
      exit(-1);
    }
  }
  ...
  if (beamInt == 0) {
    opserr << "DispBeamColumnBond2d::DispBeamColumnBond2d - failed to copy beam integration\n";
    exit(-1);
  }
  ...
  if (crdTransf == 0) {
    opserr << "DispBeamColumnBond2d::DispBeamColumnBond2d - failed to copy coordinate transformation\n";
    exit(-1);
  }
...
DispBeamColumnBond2d::DispBeamColumnBond2d()
:Element (0, ELE_TAG_DispBeamColumnBond2d),
...
int
DispBeamColumnBond2d::getNumExternalNodes() const
{
    return 2;
}

const ID&
DispBeamColumnBond2d::getExternalNodes()
{
    return connectedExternalNodes;
}

Node **
DispBeamColumnBond2d::getNodePtrs()
{
    return theNodes;
}

int
DispBeamColumnBond2d::getNumDOF()
{
    return 6;
}

void
DispBeamColumnBond2d::setDomain(Domain *theDomain)
{
    ...
    if (theNodes[0] == 0 || theNodes[1] == 0) {
      opserr << "WARNING DispBeamColumnBond2d (tag: %d), node not found in domain" << this->getTag() << endln;;
      return;
    }
	...
	//opserr << "FATAL ERROR DispBeamColumnBond2d (tag: %d), has differing number of DOFs at its nodes",
	//	this->getTag());
...
int
DispBeamColumnBond2d::commitState()
{
    int retVal = 0;

    // call element commitState to do any base class stuff
    if ((retVal = this->Element::commitState()) != 0) {
      opserr << "DispBeamColumnBond2d::commitState () - failed in base class";
    }    
...
int
DispBeamColumnBond2d::revertToLastCommit()
{
    int retVal = 0;
...
int
DispBeamColumnBond2d::revertToStart()
{
    int retVal = 0;
...
int
DispBeamColumnBond2d::update(void)
{
  int err = 0;
  ...
  if (err != 0) {
    opserr << "DispBeamColumnBond2d::update() - failed setTrialSectionDeformations()\n";
    return err;
  }
...
void
DispBeamColumnBond2d::getBasicStiff(Matrix &kb, int initial)
{
  // Zero for integral
  kb.Zero();
...
const Matrix&
DispBeamColumnBond2d::getTangentStiff()
{
...
const Matrix&
DispBeamColumnBond2d::getInitialBasicStiff()
{
  static Matrix kb(3,3);
...
const Matrix&
DispBeamColumnBond2d::getInitialStiff()
{
...
void
DispBeamColumnBond2d::zeroLoad(void)
{
  Q.Zero();
...
int 
DispBeamColumnBond2d::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  int type;
  ...
  if (type == LOAD_TAG_Beam2dUniformLoad) {
    ...
    opserr << "DispBeamColumnBond2d::DispBeamColumnBond2d -- load type unknown for element with tag: "
	   << this->getTag() << "DispBeamColumnBond2d::addLoad()\n"; 
...
int 
DispBeamColumnBond2d::addInertiaLoadToUnbalance(const Vector &accel)
{
  ...
  if (3 != Raccel1.Size() || 3 != Raccel2.Size()) {
    opserr << "DispBeamColumnBond2d::addInertiaLoadToUnbalance matrix and vector sizes are incompatible\n";
    return -1;
  }
...
const Vector&
DispBeamColumnBond2d::getResistingForce()
{
  double L = crdTransf->getInitialLength();
...
const Vector&
DispBeamColumnBond2d::getResistingForceIncInertia()
{
...
int
DispBeamColumnBond2d::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  if (theChannel.sendVector(dbTag, commitTag, data) < 0) {
    opserr << "DispBeamColumnBond2d::sendSelf() - failed to send data Vector\n";
     return -1;
  }
  
  // send the coordinate transformation
  if (crdTransf->sendSelf(commitTag, theChannel) < 0) {
     opserr << "DispBeamColumnBond2d::sendSelf() - failed to send crdTranf\n";
     return -1;
  }      

  // send the beam integration
  if (beamInt->sendSelf(commitTag, theChannel) < 0) {
    opserr << "DispBeamColumnBond2d::sendSelf() - failed to send beamInt\n";
    return -1;
  }      
  ...
  if (theChannel.sendID(dbTag, commitTag, idSections) < 0)  {
    opserr << "DispBeamColumnBond2d::sendSelf() - failed to send ID data\n";
    return -1;
  }    
  ...
  for (j = 0; j<numSections; j++) {
    if (theSections[j]->sendSelf(commitTag, theChannel) < 0) {
      opserr << "DispBeamColumnBond2d::sendSelf() - section " << 
...
int
DispBeamColumnBond2d::recvSelf(int commitTag, Channel &theChannel,
			   FEM_ObjectBroker &theBroker)
{
  ...
  if (theChannel.recvVector(dbTag, commitTag, data) < 0)  {
    opserr << "DispBeamColumnBond2d::recvSelf() - failed to recv data Vector\n";
    return -1;
  }
  ...
  if (crdTransf == 0 || crdTransf->getClassTag() != crdTransfClassTag) {
      ...
      if (crdTransf == 0) {
	opserr << "DispBeamColumnBond2d::recvSelf() - failed to obtain a CrdTrans object with classTag " <<
  ...
  if (crdTransf->recvSelf(commitTag, theChannel, theBroker) < 0) {
    opserr << "DispBeamColumnBond2d::sendSelf() - failed to recv crdTranf\n";
    return -3;
  }      
  ...
  if (beamInt == 0 || beamInt->getClassTag() != beamIntClassTag) {
      ...
      if (beamInt == 0) {
	opserr << "DispBeamColumnBond2d::recvSelf() - failed to obtain the beam integration object with classTag" <<
  ...
  {
     opserr << "DispBeamColumnBond2d::sendSelf() - failed to recv beam integration\n";
     return -3;
  }      
  ...
  if (theChannel.recvID(dbTag, commitTag, idSections) < 0)  {
    opserr << "DispBeamColumnBond2d::recvSelf() - failed to recv ID data\n";
    return -1;
  }    
  ...
  if (numSections != nSect) {
    ...
    if (theSections == 0) {
opserr << "DispBeamColumnBond2d::recvSelf() - out of memory creating sections array of size " <<
  nSect << endln;
      return -1;
    }    
    ...
    for (i=0; i<numSections; i++) {
      ...
      if (theSections[i] == 0) {
	opserr << "DispBeamColumnBond2d::recvSelf() - Broker could not create Section of class type " <<
	  sectClassTag << endln;
	exit(-1);
      }
      theSections[i]->setDbTag(sectDbTag);
      if (theSections[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
	opserr << "DispBeamColumnBond2d::recvSelf() - section " << i << " failed to recv itself\n";
	return -1;
      }     
    }
...
void
DispBeamColumnBond2d::Print(OPS_Stream &s, int flag)
{
  if (flag == OPS_PRINT_CURRENTSTATE) {
    s << "\nDispBeamColumnBond2d, element id:  " << this->getTag() << endln;
...
int
DispBeamColumnBond2d::displaySelf(Renderer &theViewer, int displayMode, float fact, const char **displayModes, int numModes)
{
    static Vector v1(3);
    static Vector v2(3);
...
Response*
DispBeamColumnBond2d::setResponse(const char **argv, int argc,
			      OPS_Stream &output)
{
  Response *theResponse = 0;

  output.tag("ElementOutput");
  output.attr("eleType","DispBeamColumnBond2d");
...
int 
DispBeamColumnBond2d::getResponse(int responseID, Information &eleInfo)
{
  double V;
  double L = crdTransf->getInitialLength();
...
int 
DispBeamColumnBond2d::getResponseSensitivity(int responseID, int gradNumber,
					 Information &eleInfo)
{
  // Basic deformation sensitivity
  if (responseID == 3) {  
...
int
DispBeamColumnBond2d::setParameter(const char **argv, int argc, Parameter &param)
{
  if (argc < 1)
    return -1;
...
int
DispBeamColumnBond2d::updateParameter (int parameterID, Information &info)
{
  if (parameterID == 1) {
    rho = info.theDouble;
    return 0;
...
int
DispBeamColumnBond2d::activateParameter(int passedParameterID)
{
...
const Matrix &
DispBeamColumnBond2d::getInitialStiffSensitivity(int gradNumber)
{
  static Matrix kb(3,3);
...
const Matrix &
DispBeamColumnBond2d::getMassSensitivity(int gradNumber)
{
  K.Zero();
...
const Vector &
DispBeamColumnBond2d::getResistingForceSensitivity(int gradNumber)
{
  double L = crdTransf->getInitialLength();
  double oneOverL = 1.0/L;
...
// NEW METHOD
int
DispBeamColumnBond2d::commitSensitivity(int gradNumber, int numGrads)
{
