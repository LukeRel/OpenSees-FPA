...
// $Source: /usr/local/cvs/OpenSees/SRC/element/forceBeamColumn/ForceBeamColumnCons3d.cpp,v $
...
Modified by Luca Parente
Testing consistent only force based beam element based on Spacone, Ciampi 1996
...
#include <Parameter.h>
#include <ForceBeamColumnCons3d.h>
...
Matrix ForceBeamColumnCons3d::theMatrix(12,12);
Vector ForceBeamColumnCons3d::theVector(12);
double ForceBeamColumnCons3d::workArea[200];
/*
Vector ForceBeamColumnCons3d::vsSubdivide[maxNumSections];
Matrix ForceBeamColumnCons3d::fsSubdivide[maxNumSections];
Vector ForceBeamColumnCons3d::SsrSubdivide[maxNumSections];
*/

void* OPS_ForceBeamColumnCons3d()
{
    ...
    Element *theEle =  new ForceBeamColumnCons3d(iData[0],iData[1],iData[2],secTags.Size(),sections,
...
ForceBeamColumnCons3d::ForceBeamColumnCons3d() :
    ...
    fs(0), e(0), Ssr(0), Ssu(0), vscommit(0),
...
ForceBeamColumnCons3d::ForceBeamColumnCons3d (int tag, int nodeI, int nodeJ,
				      int numSec, SectionForceDeformation **sec,
				      BeamIntegration &bi,
				      CrdTransf &coordTransf, double massDensPerUnitLength,
				      int maxNumIters, double tolerance):
  Element(tag,ELE_TAG_ForceBeamColumnCons3d), connectedExternalNodes(2),
  beamIntegr(0), numSections(0), sections(0), crdTransf(0),
  ...
  Ki(0), isTorsion(false), parameterID(0), maxSubdivisions(10)
{
  ...
  if (beamIntegr == 0) {
    opserr << "Error: ForceBeamColumnCons3d::ForceBeamColumnCons3d: could not create copy of beam integration object" << endln;
    exit(-1);
  }
  ...
  if (crdTransf == 0) {
    opserr << "Error: ForceBeamColumnCons3d::ForceBeamColumnCons3d: could not create copy of coordinate transformation object" << endln;
    exit(-1);
  }
...
int
ForceBeamColumnCons3d::getNumExternalNodes(void) const
{
  return 2;
}

const ID &
ForceBeamColumnCons3d::getExternalNodes(void) 
{
  return connectedExternalNodes;
}

Node **
ForceBeamColumnCons3d::getNodePtrs()
{
  return theNodes;
}

int
ForceBeamColumnCons3d::getNumDOF(void) 
{
  return NEGD;
}

void
ForceBeamColumnCons3d::setDomain(Domain *theDomain)
{
  ...
  if (theDomain == 0) {
    ...
    opserr << "ForceBeamColumnCons3d::setDomain:  theDomain = 0 ";
    exit(0); 
  }
  ...
  if (theNodes[0] == 0) {
    opserr << "ForceBeamColumnCons3d::setDomain: Nd1: ";
    opserr << Nd1 << "does not exist in model\n";
    exit(0);
  }
  
  if (theNodes[1] == 0) {
    opserr << "ForceBeamColumnCons3d::setDomain: Nd2: ";
  ...
  if ((dofNode1 != NND) || (dofNode2 != NND)) {
    opserr << "ForceBeamColumnCons3d::setDomain(): Nd2 or Nd1 incorrect dof ";
    exit(0);
  }
   
  // initialize the transformation
  if (crdTransf->initialize(theNodes[0], theNodes[1])) {
    opserr << "ForceBeamColumnCons3d::setDomain(): Error initializing coordinate transformation";  
    exit(0);
  }
  ...
  if (L == 0.0) {
    opserr << "ForceBeamColumnCons3d::setDomain(): Zero element length:" << this->getTag();  
    exit(0);
  }
  ...
  opserr << "setdomain done" << endln;
}

int
ForceBeamColumnCons3d::commitState()
{
  int err = 0;
  int i = 0;
  ...
  if ((err = this->Element::commitState()) != 0) {
    opserr << "ForceBeamColumnCons3d::commitState () - failed in base class";
  }    
...
int ForceBeamColumnCons3d::revertToLastCommit()
{
  int err;
  int i = 0;
  ...
  do {
    ...
    sections[i]->setTrialSectionDeformation(e[i]);      
...
int ForceBeamColumnCons3d::revertToStart()
{
...
ForceBeamColumnCons3d::getInitialStiff(void)
{
  ...
  if (f.Solve(I, kvInit) < 0)
    opserr << "ForceBeamColumnCons3d::getInitialStiff() -- could not invert flexibility";
...
void
ForceBeamColumnCons3d::computeReactions(double *p0)
{
  int type;
  double L = crdTransf->getInitialLength();
...
void
ForceBeamColumnCons3d::computeReactionSensitivity(double *dp0dh, int gradNumber)
{
  int type;
  double L = crdTransf->getInitialLength();
...
const Vector &
ForceBeamColumnCons3d::getResistingForce(void)
{
...
{
    opserr << "initializeSectionHistoryVariables started" << endln;
        for (int i = 0; i < numSections; i++) {
    ...
    opserr << "initializeSectionHistoryVariables finished" << endln;
}
...
int ForceBeamColumnCons3d::update()
{
    ...
    static Vector r(NEBD);        // element residual displacements (r)
    static Matrix f(NEBD, NEBD);   // element flexibility matrix (f)
    static Matrix I(NEBD, NEBD);   // an identity matrix for matrix inverse
    int i, j;

    I.Zero();
    for (i = 0; i < NEBD; i++) I(i, i) = 1.0;
    ...
    // calculate nodal force increments and update nodal forces --->       SeTrial = Se_k + kv_k * dv
    ...
    // Adding element forces to residuals directly if there are elastic parts
    // This is not the case, so commenting everything
    // [...]

    // SECTION INTEGRATION ---------------------------------------------------------------------------------------------------------------------------
    for (i = 0; i < numSections; i++) {

        int order = sections[i]->getOrder(); // Equals 6 for NDFiber, equals 4 for Fiber
        const ID& code = sections[i]->getType();

        static Vector s;       // Section trial forces
        static Vector ds;      // Section resisting forces increments
        static Vector de;      // Section deformation for integration
        static Vector t;       // Section residual deformation
        static Matrix fb;      // Section flexibility for integration

        s.setData(workArea, order);
        ds.setData(&workArea[order], order);
        de.setData(&workArea[2*order], order);
        t.setData(&workArea[3*order], order);
        fb.setData(&workArea[4*order], order, NEBD);

        // Section i location and weight
        double xL = xi[i];
        double xL1 = xL - 1.0;
        double wtL = wt[i] * L;
        ...
        // TRIAL SECTION FORCES [total] --->                          s = b * Se + Sq
        for (ii = 0; ii < order; ii++) {
            ...
            case SECTION_RESPONSE_MZ:
                s(ii) = xL1 * SeTrial(1) + xL * SeTrial(2);
                break;
            case SECTION_RESPONSE_VY:
                s(ii) = oneOverL * (SeTrial(1) + SeTrial(2));
                break;
            case SECTION_RESPONSE_MY:
                s(ii) = xL1 * SeTrial(3) + xL * SeTrial(4);
                break;
            case SECTION_RESPONSE_VZ:
                s(ii) = oneOverL * (SeTrial(3) + SeTrial(4));
                break;
        ...
        if (numEleLoads > 0) this->computeSectionForces(s, i);

        // SECTION FORCES [incremental] --->                          ds = b * dSe + Ssu[i]
        for (ii = 0; ii < order; ii++) {
            switch (code(ii)) {
            case SECTION_RESPONSE_P:
                ds(ii) = dSe(0);
        ...
        ds.addVector(1.0, Ssu[i], 1.0);

        // SECTION DEFORMATIONS [total] [regular newton] --->         de = fs * ds; e = e_k + de
        de.addMatrixVector(0.0, fs[i], ds, 1.0);
        if (initialFlag != 0) e[i] += de;

        // Set section deformation: this starts the section method which returns forces and flexibility
        if (sections[i]->setTrialSectionDeformation(e[i]) < 0) {
        ...
        ds = s;
        ds.addVector(1.0, Ssr[i], -1.0);  //       dSs = Ss - Ssr[i];
        t.addMatrixVector(0.0, fs[i], ds, 1.0);
        ...
        // integrate residual deformations                             r += (b' t) * wtL;
        dv = t;
        t.addVector(1.0, e[i], 1.0);
        double dei;
        for (ii = 0; ii < order; ii++) {
            dei = dv(ii) * wtL;
            switch (code(ii)) {
    ...
    if (!isTorsion) {
        ...
        r(5) = SeTrial(5) * DefaultLoverGJ;
    }

    // Calculate element stiffness matrix ---------------------------- K = inv(F)
    if (f.Solve(I, kv) < 0) opserr << "ForceBeamColumnCons3d::update() -- could not invert flexibility\n";

    // Calculate new Se ---------------------------------------------- Se = Setrial - Kr
    Se = SeTrial;
    Se.addMatrixVector(1.0, kv, r, -1.0);

    // Calculate new section residual -------------------------------- Ssu[i] = bq - Ssr[i]
    for (i = 0; i < numSections; i++) {
        
        int order = sections[i]->getOrder(); // Equals 6 for NDFiber, equals 4 for Fiber
        const ID& code = sections[i]->getType();

        static Vector su;      // Section unbalanced forces (s-sr)
        su.setData(workArea, order);
        ...
        for (int ii = 0; ii < order; ii++) {
            ...
            case SECTION_RESPONSE_P:
                ...
                break;
            case SECTION_RESPONSE_MZ:
                su(ii) = xL1 * Se(1) + xL * Se(2);
                break;
            case SECTION_RESPONSE_VY:
                su(ii) = oneOverL * (Se(1) + Se(2));
                break;
            case SECTION_RESPONSE_MY:
                su(ii) = xL1 * Se(3) + xL * Se(4);
                break;
            case SECTION_RESPONSE_VZ:
                su(ii) = oneOverL * (Se(3) + Se(4));
                break;
            case SECTION_RESPONSE_T:
                su(ii) = Se(5);
                break;
  ...
  void ForceBeamColumnCons3d::getDistrLoadInterpolatMatrix(double xi, Matrix &bp, const ID &code)
  {
    bp.Zero();
  ...
  const Matrix &
  ForceBeamColumnCons3d::getMass(void)
  { 
    theMatrix.Zero();
  ...
  void 
  ForceBeamColumnCons3d::zeroLoad(void)
  {
    load.Zero();
...
int
ForceBeamColumnCons3d::addLoad(ElementalLoad *theLoad, double loadFactor)
{
...
void
ForceBeamColumnCons3d::computeSectionForces(Vector &sp, int isec)
{
  int type;
...
void
ForceBeamColumnCons3d::computeSectionForceSensitivity(Vector &dspdh, int isec,
						  int gradNumber)
{
  int type;
  ...
  for (int i = 0; i < numEleLoads; i++) {
	...
	if (x <= a) {
      ...
      opserr << "ForceBeamColumnCons3d::computeSectionForceSensitivity -- load type unknown for element with tag: " <<
	this->getTag() << endln;
    }
  }
}
  ...
  int 
  ForceBeamColumnCons3d::addInertiaLoadToUnbalance(const Vector &accel)
  {
  ...
  const Vector &
  ForceBeamColumnCons3d::getResistingForceIncInertia()
  {	
  ...
  int
  ForceBeamColumnCons3d::sendSelf(int commitTag, Channel &theChannel)
  {  
    ...
    if (theChannel.sendID(dbTag, commitTag, idData) < 0) {
      opserr << "ForceBeamColumnCons3d::sendSelf() - failed to send ID data\n";
      return -1;
    }    
    ...
    if (crdTransf->sendSelf(commitTag, theChannel) < 0) {
      opserr << "ForceBeamColumnCons3d::sendSelf() - failed to send crdTranf\n";
      return -1;
    }      

    if (beamIntegr->sendSelf(commitTag, theChannel) < 0) {
      opserr << "ForceBeamColumnCons3d::sendSelf() - failed to send beamIntegr\n";
      return -1;
    }      
	...
	opserr << "ForceBeamColumnCons3d::sendSelf() - section " <<
    ...
    if (theChannel.sendVector(dbTag, commitTag, dData) < 0) {
       opserr << "ForceBeamColumnCons3d::sendSelf() - failed to send Vector data\n";
  ...
  int
  ForceBeamColumnCons3d::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
  {
    //
    ...
    if (theChannel.recvID(dbTag, commitTag, idData) < 0)  {
      opserr << "ForceBeamColumnCons3d::recvSelf() - failed to recv ID data\n";
	...
	if (crdTransf == 0) {
	    opserr << "ForceBeamColumnCons3d::recvSelf() - failed to obtain a CrdTrans object with classTag" <<
    ...
    {
       opserr << "ForceBeamColumnCons3d::sendSelf() - failed to recv crdTranf\n";
       return -3;
    }      
	...
	if (beamIntegr == 0) {opserr << "ForceBeamColumnCons3d::recvSelf() - failed to obtain the beam integration object with classTag" <<
    ...
    {
       opserr << "ForceBeamColumnCons3d::sendSelf() - failed to recv beam integration\n";
	...
	delete [] vscommit;
      ...
      if (vscommit == 0) {
	opserr << "ForceBeamColumnCons3d::recvSelf -- failed to allocate vscommit array\n";
	return -1;
      }
	...
	delete [] fs;
      ...
      if (fs == 0) {
	opserr << "ForceBeamColumnCons3d::recvSelf -- failed to allocate fs array\n";
	return -1;
      }
	...
	delete [] e;
      ...
      // Allocate the right number
      e = new Vector[numSections];  
      if (e == 0) {
	opserr << "ForceBeamColumnCons3d::recvSelf -- failed to allocate vs array\n";
	return -1;
      }
	...
	return -1;
      ...
      if (sections == 0) {
	opserr << "ForceBeamColumnCons3d::recvSelf() - out of memory creating sections array of size" <<
	  idData(3) << endln;
	exit(-1);
      }    
	...
	if (sections[i] == 0) {
	  opserr << "ForceBeamColumnCons3d::recvSelf() - Broker could not create Section of class type" <<
	    sectClassTag << endln;
	  exit(-1);
	}
	...
	if (sections[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
	  opserr << "ForceBeamColumnCons3d::recvSelf() - section " << 
    ...
    if (theChannel.recvVector(dbTag, commitTag, dData) < 0)  {
      opserr << "ForceBeamColumnCons3d::recvSelf() - failed to send Vector data\n";
      return -1;
    }    
  ...
  int
  ForceBeamColumnCons3d::getInitialFlexibility(Matrix &fe)
  {
    fe.Zero();
...
int
ForceBeamColumnCons3d::getInitialDeformations(Vector &v0)
{
  v0.Zero();
  if (numEleLoads < 1)
      return 0;
  ...
  void
      ForceBeamColumnCons3d::compSectionDisplacements(Vector sectionCoords[],
          Vector sectionDispls[]) const
  {
      ...
      for (i = 0; i < numSections; i++) {
          ...
          if (sectionKey1 == 0) {
              opserr << "FATAL ForceBeamColumnCons3d::compSectionResponse - section does not provide Mz response\n";
              exit(-1);
          }
          if (sectionKey2 == 0) {
              opserr << "FATAL ForceBeamColumnCons3d::compSectionResponse - section does not provide My response\n";
              exit(-1);
          }
  ...
  void
  ForceBeamColumnCons3d::Print(OPS_Stream &s, int flag)
  {
    ...
    // flag set to 2 used to print everything .. used for viewing data for UCSD renderer  
	 ...
	 if (!coords) {
	   opserr << "ForceBeamColumnCons3d::Print() -- failed to allocate coords array";   
	   exit(-1);
	 }
    ...
    if (flag == OPS_PRINT_CURRENTSTATE) {
       s << "\nElement: " << this->getTag() << " Type: ForceBeamColumnCons3d ";
  ...
  OPS_Stream &operator<<(OPS_Stream &s, ForceBeamColumnCons3d &E)
  {
    E.Print(s);
    return s;
  }
  ...
  int
  ForceBeamColumnCons3d::displaySelf(Renderer &theViewer, int displayMode, float fact, const char** displayModes, int numModes)
  {
    static Vector v1(3);
    static Vector v2(3);
  ...
  Response*
  ForceBeamColumnCons3d::setResponse(const char **argv, int argc, OPS_Stream &output)
  {
    Response *theResponse = 0;
    
    output.tag("ElementOutput");
    output.attr("eleType","ForceBeamColumnCons3d");
...
int 
ForceBeamColumnCons3d::getResponse(int responseID, Information &eleInfo)
{
  static Vector vp(6);
  static Matrix fe(6,6);
  ...
  // Tangent drift
	...
	if(!indata) { // file couldn't be opened
		opserr << "ForceBeamColumnCons3d::getResponse()"
...
int 
ForceBeamColumnCons3d::getResponseSensitivity(int responseID, int gradNumber,
					  Information &eleInfo)
{
...
int
ForceBeamColumnCons3d::setParameter(const char **argv, int argc, Parameter &param)
{
  if (argc < 1)
    return -1;
...
int
ForceBeamColumnCons3d::updateParameter (int parameterID, Information &info)
{
  if (parameterID == 1) {    
...
int
ForceBeamColumnCons3d::activateParameter(int passedParameterID)
{
...
const Matrix&
ForceBeamColumnCons3d::getKiSensitivity(int gradNumber)
{
  theMatrix.Zero();
  return theMatrix;
}
...
const Matrix&
ForceBeamColumnCons3d::getMassSensitivity(int gradNumber)
{
    theMatrix.Zero();
...
const Vector&
ForceBeamColumnCons3d::getResistingForceSensitivity(int gradNumber)
{
  static Vector dqdh(6);
...
int
ForceBeamColumnCons3d::commitSensitivity(int gradNumber, int numGrads)
{
  int err = 0;
...
const Vector &
ForceBeamColumnCons3d::computedqdh(int gradNumber)
{
...
const Matrix&
ForceBeamColumnCons3d::computedfedh(int gradNumber)
{
  static Matrix dfedh(6,6);
...
void
ForceBeamColumnCons3d::setSectionPointers(int numSec, SectionForceDeformation **secPtrs)
{
  if (numSec > maxNumSections) {
    opserr << "Error: ForceBeamColumnCons3d::setSectionPointers -- max number of sections exceeded";
  }
  
  numSections = numSec;
  
  if (secPtrs == 0) {
    opserr << "Error: ForceBeamColumnCons3d::setSectionPointers -- invalid section pointer";
  }	  
  
  sections = new SectionForceDeformation *[numSections];
  if (sections == 0) {
    opserr << "Error: ForceBeamColumnCons3d::setSectionPointers -- could not allocate section pointers";
  }  
  
  for (int i = 0; i < numSections; i++) {
    
    if (secPtrs[i] == 0) {
      opserr << "Error: ForceBeamColumnCons3d::setSectionPointers -- null section pointer " << i << endln;
    }
    
    sections[i] = secPtrs[i]->getCopy();
    
    if (sections[i] == 0) {
      opserr << "Error: ForceBeamColumnCons3d::setSectionPointers -- could not create copy of section " << i << endln;
    }
  ...
  if (!isTorsion)
    opserr << "ForceBeamColumnCons3d::ForceBeamColumnCons3d -- no torsion detected in sections, " <<
  ...
  fs  = new Matrix [numSections];
  if (fs == 0) {
    opserr << "ForceBeamColumnCons3d::setSectionPointers -- failed to allocate fs array";
  }
  
  e = new Vector [numSections];
  if (e == 0) {
    opserr << "ForceBeamColumnCons3d::setSectionPointers -- failed to allocate e array";
  }
  
  Ssr  = new Vector [numSections];
  if (Ssr == 0) {
    opserr << "ForceBeamColumnCons3d::setSectionPointers -- failed to allocate Ssr array";
  }

  Ssu = new Vector[numSections];
  if (Ssu == 0) {
      opserr << "ForceBeamColumnCons3d::setSectionPointers -- failed to allocate Ssu array";
  }

  vscommit = new Vector [numSections];
  if (vscommit == 0) {
    opserr << "ForceBeamColumnCons3d::setSectionPointers -- failed to allocate vscommit array";   
  }
